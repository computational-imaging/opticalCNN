...
class ClassificationModel(abc.ABC):
    """Generic tensorflow classification model class.
    """
    ...
    def fit(self,
        ...
        # x_test, y_test = self._get_validation_queue()
        ...
        with tf.variable_scope('model'):
            ...
            data_loss_graph, accuracy_graph = self._get_data_loss(model_output_train, y_train)
        ...
        try:
            for step in range(num_steps):
                _, total_loss, reg_loss, data_loss, acc = self.sess.run([train_step,
                                                                    total_loss_graph, reg_loss_graph,
                                                                    data_loss_graph, accuracy_graph])
                if not step % 25:
                    print("Step %d: total_loss %0.6f,  reg_loss %0.6f,  data_loss %0.6f,  acc %0.2f\n"%\
                            (step, total_loss, reg_loss, data_loss, acc))
