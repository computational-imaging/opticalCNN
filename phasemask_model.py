...
class PhaseMaskModel(model.Model):
    ...
    def _get_training_queue(self, batch_size, num_threads=4):
        ...
        file_list = tf.matching_files('./testing/mnist_pad_00*.png')
        ...
        psf = tf.convert_to_tensor(np.load('testing/doublepsf_larger.npy'), tf.float32)
...
if __name__=='__main__':
    ...
    phasemask.fit(model_params = {'hm_reg_scale':1e-1},
                   ...
                   logdir = os.path.join('checkpoints/doublepsf/', now.strftime('%Y%m%d-%H%M%S/')),
                   num_steps = num_steps)
